...
class GlobalPositioner:
    ...
    def run(
        ...
        if self._save_iteration_visualization:
            ...
            # Pipeline visualizer: dump per-iter COLMAP folders so the GP convergence
            # (random init → emerging structure) becomes the FIRST act of playback.
            # Lands in <output_root>/pipeline_trace/ alongside BA trace dumps.
            # Subsamples to ~21 frames (first 20 LM iters one-for-one — that's where
            # the dramatic motion happens — plus the final iter for a clean handoff).
            if output_root is not None:
                self._dump_pipeline_trace(
                    output_root=output_root,
                    positions_trace=positions_trace,
    ...
    def _dump_pipeline_trace(
        ...
        """Dump GP per-LM-iter snapshots as COLMAP-format folders for the visualizer.

        Subsamples to first 20 LM iters one-for-one (where the dramatic random→structure
        motion happens) plus the absolute final iter (clean handoff to the BA stages).
        Image colors are sampled once from the source images and reused across all GP
        iters so each track has a stable identity color across the convergence animation.
        """
        import shutil
        from gtsfm.visualization.pipeline_trace import (
            append_to_manifest, dump_gp_iter, get_default_sampler,
        )

        class _Track2dAdapter:
            """Wrap SfmTrack2d so ImageColorSampler's `.measurement(k)` interface works.
            SfmTrack2d uses `.measurements` list of (i, uv); BA's SfmTrack uses `.measurement(k)`."""
            def __init__(self, t2d):
                self.t2d = t2d
            def numberMeasurements(self):
                return len(self.t2d.measurements)
            def measurement(self, k):
                m = self.t2d.measurements[k]
                return (m.i, m.uv)

        # Match the trace BA's `<output_root>/pipeline_trace` so GP + BA dumps land in
        # the same directory and the visualizer's manifest plays them as one timeline.
        trace_dir = Path(output_root) / "pipeline_trace"
        shutil.rmtree(trace_dir, ignore_errors=True)
        trace_dir.mkdir(parents=True, exist_ok=True)

        # Pre-sample per-track colors ONCE so each point has a stable identity across
        # all GP iters — viewer can visually follow a single point as it converges.
        track_colors_map: dict[int, list[int]] = {}
        sampler = get_default_sampler()
        if sampler is not None:
            sampler.prime_scale((_Track2dAdapter(t2d) for t2d in filtered_tracks))
            n_sampled = 0
            for ti, t2d in enumerate(filtered_tracks):
                if ti in track_indices:
                    try:
                        color = sampler.sample_track(_Track2dAdapter(t2d))
                        track_colors_map[ti] = color
                        if color != [128, 128, 128]:
                            n_sampled += 1
                    except Exception:
                        pass
            logger.info("[pipeline_trace/gp] sampled colors for %d/%d tracks",
                        n_sampled, len(filtered_tracks))
        else:
            logger.warning("[pipeline_trace/gp] no image color sampler — set "
                           "GTSFM_PIPELINE_TRACE_IMAGES_DIR to enable colored point trails")

        GP_HEAD_FRAMES = 20
        head = list(range(min(GP_HEAD_FRAMES, len(positions_trace))))
        last_iter = len(positions_trace) - 1
        if last_iter not in head:
            head.append(last_iter)

        gp_dump_count = 0
        for k in head:
            frame = positions_trace[k]
            subdir = f"stage_gp_{gp_dump_count:03d}"
            try:
                dump_gp_iter(
                    cam_positions=frame["cameras"],
                    wRi_matrices=wRi_matrices,
                    landmark_positions=frame["landmarks"],
                    stage_dir=trace_dir / subdir,
                    stage_name=f"gp_iter_{k:03d}",
                    iteration=k,
                    extra_metadata={
                        "error": float(error_trace[k]) if k < len(error_trace) else 0.0,
                        "num_landmarks": int(len(frame["landmarks"])),
                    },
                    track_colors=track_colors_map if track_colors_map else None,
                )
                append_to_manifest(
                    trace_dir, subdir, f"gp_iter_{k:03d}", k,
                    {"phase": "gp",
                     "error": float(error_trace[k]) if k < len(error_trace) else 0.0},
                )
                gp_dump_count += 1
            except Exception as e:
                logger.warning("[pipeline_trace/gp] dump iter %d failed: %s", k, e)
        logger.info("[pipeline_trace/gp] dumped %d GP iter snapshots → %s",
                    gp_dump_count, trace_dir)
