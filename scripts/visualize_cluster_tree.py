...
def _build_loader(
    ...
    if enable_no_exif:
        overrides.append("enable_no_exif=true")
  ...
  <script>
    ...
    function graphDataForNode(node, cameraIndices, edgeScope) {{
      ...
      for (const idx of clippedNodes) {{
        const key = String(idx);
        const entry = THUMB_DATA[key] || {{}};
        const nodeData = {{ id: `n_${{idx}}`, camId: idx, label: String(idx) }};
        if (entry.thumbnail_data_uri) {{
          nodeData.img = entry.thumbnail_data_uri;
        }}
...
def _parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="Pass enable_no_exif=true to loader (load all images regardless of EXIF). "
        "Required for datasets processed with this flag (e.g. Dubrovnik).",
    )
    ...
    if args.loader_config is not None:
        ...
        if hydra_available:
            loader = _build_loader(
                ...
                enable_no_exif=args.enable_no_exif,
            )
